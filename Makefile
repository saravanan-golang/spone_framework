...
graph:
	...
	@cp -f cmd/user_service/main.go .
	go-callvis -skipbrowser -format=svg -nostd -file=user_service user_module
	@rm -f main.go user_service.gv
...
.PHONY: build
# Build user_service for linux amd64 binary
build:
	@echo "building 'user_service', linux binary file will output to 'cmd/user_service'"
	@cd cmd/user_service && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build
